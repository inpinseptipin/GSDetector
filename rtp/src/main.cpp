...
constexpr uint32_t PAYLOAD_MAXLEN = (512);
constexpr int TEST_PACKETS = 1;
...
int main(void)
{
    ...
    auto buffer = AuxPort::Utility::generateRandomValues<uint8_t>(4,0,255);
    AuxPort::Logger::Log("Logging Audio Buffer");
    AuxPort::Logger::Log(buffer);
    ...
    if (send)
    {
       ...
       * it will reconstruct the full sent frame from fragments when all fragments have been received */
        auto media = std::unique_ptr<uint8_t[]>(new uint8_t[4]);
        ...
        for (int i = 0; i < TEST_PACKETS; ++i)
        {
            ...
            std::cout << "Sending RTP frame " << i + 1 << '/' << TEST_PACKETS
                << ". Payload size: " << 4 << std::endl;

            if (send->push_frame(media.get(), 4, RTP_NO_FLAGS) != RTP_OK)
            {
                cleanup(ctx, local_session, remote_session, send, recv);
...
void rtp_receive_hook(void* arg, uvgrtp::frame::rtp_frame* frame)
{
    ...
    audioData.resize(frame->payload_len);
    for (uint32_t i = 0; i < frame->payload_len; i++)
    {
        audioData[i] = payload[i];
    }
    AuxPort::Logger::Log(audioData);
