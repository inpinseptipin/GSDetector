...
def train_val_one_epoch(
    ...
    with tqdm(
        train_loader,
        desc=f"Epoch {epoch+1}/{num_epochs} | LR = {scheduler.get_last_lr()[0]:.2E}",
        unit="batch",
...
def main() -> None:
    ...
    total_indices = torch.randperm(len(full_dataset), generator=generator)
    ...
    train_dataset, valid_dataset, test_dataset = random_split(
        shuffled_dataset, [train_size, valid_size, test_size], generator=generator
    )

    # Normalize
    mean = np.mean([torch.mean(spec) for spec, _ in train_dataset])
    std = np.std([torch.std(spec) for spec, _ in train_dataset])
    ...
    writer = SummaryWriter(
        f"runs/{model_name}_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    )
    layout = {
        "Metrics": {
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(
            f"Validation: Accuracy = {val_acc*100:.4f}, Avg loss = {val_loss:.8f}",
            end=" ",
        )
        scheduler.step()
        writer.flush()
    ...
    test_loss, test_acc, y_prob, y_true = evaluate(
    ...
    print(
        metrics.classification_report(
            ...
            y_pred,
            digits=6,
            target_names=["negative samples", "positive samples"],
        )
    )
